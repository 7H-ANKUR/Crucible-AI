...
---------------------------------
Thin adapter between the API layer and Crucible AI frozen core dataset modules.
"""
...
from api.services.dataframe_io import load_dataframe, load_dataframe_from_record
...
class DatasetService:
    ...
    def _load_df(self, path: Path, ext: str) -> pd.DataFrame:
        return load_dataframe(path, ext)

    def _load_df_from_record(self, record: dict[str, Any]) -> pd.DataFrame:
        return load_dataframe_from_record(record)
