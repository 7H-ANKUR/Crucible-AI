...
tests/test_api.py
-----------------
Comprehensive API tests for Crucible AI FastAPI layer (Phase N).
...
def client(tmp_store):
    ...
    os.environ["CRUCIBLE_STORE_ROOT"] = str(tmp_store / "store")
    os.environ["CRUCIBLE_UPLOAD_TMP"] = str(tmp_store / "uploads")
    os.environ["CRUCIBLE_CHECKPOINT_ROOT"] = str(tmp_store / "checkpoints")
    os.environ["CRUCIBLE_ARTIFACT_ROOT"] = str(tmp_store / "artifacts")
    os.environ["CRUCIBLE_CORS_ORIGINS"] = "http://localhost:3000"
    os.environ["CRUCIBLE_MAX_UPLOAD_MB"] = "1"
...
def assert_envelope(resp, *, status: str = "success"):
    """Assert response has standard Crucible AI envelope shape."""
...
class TestHealth:
    def test_health_200(self, client):
        ...
        assert body["service"] == "crucible-api"
