"""app/api/core/cache.py — Crucible AI Production-Grade Hybrid Tiered Cache.
...
logger = logging.getLogger("crucible.cache")
...
class RedisCircuitBreaker:
    """Strict circuit breaker preventing cascading socket hangs on an impaired dependency.

    Written for Redis and still the cache's breaker, but the logic is generic: the
    Crucible AI engine bridge uses it too, passing its own ``name`` so the logs say which
    dependency tripped. Import it as ``CircuitBreaker`` in new code.
    """
        ...
        with self._lock:
            ...
            if self.state == CircuitState.OPEN:
                if now - self.last_state_change_mono >= self.cooldown_seconds:
                    logger.info("%s circuit breaker entering HALF_OPEN probe state.", self.name)
    ...
    def record_success(self) -> None:
        with self._lock:
            if self.state in (CircuitState.HALF_OPEN, CircuitState.OPEN):
                logger.info("%s circuit breaker probe succeeded. Closing circuit (CLOSED).", self.name)
            self.state = CircuitState.CLOSED
            self.failure_count = 0
    ...
    def record_failure(self, error: Exception | None = None) -> None:
        with self._lock:
            ...
            if self.state == CircuitState.CLOSED and self.failure_count >= self.failure_threshold:
                ...
                self.last_state_change_mono = now
                logger.warning(
                    "%s circuit breaker tripped OPEN after %d failures. Cooldown: %ss. Last error: %s",
                    self.name, self.failure_count, self.cooldown_seconds, error
                )
            elif self.state == CircuitState.HALF_OPEN:
                self.state = CircuitState.OPEN
                self.last_state_change_mono = now
                logger.warning("%s circuit breaker probe failed. Re-opening circuit (OPEN). Error: %s", self.name, error)


#: Generic alias. New callers (e.g. the Crucible AI bridge) should use this name.
...
def build_cache_key(
    ...
    Format:
      crucible:cache:v{version}:{domain}:{resource}:{entity_id}:{params_hash}:{model_version}:{dataset_version}:{auth_scope}
    """
    version = getattr(settings, "CACHE_SCHEMA_VERSION", 1)
    parts = ["crucible", "cache", f"v{version}", domain, resource]
...
class TieredCacheManager:
    ...
    def __init__(
        ...
        self._swr_executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=4, thread_name_prefix="crucible_swr_worker"
        )
    ...
    def invalidate_domain(self, domain: str) -> int:
        ...
        pattern = f"crucible:cache:v{ver}:{domain}:*"
    ...
    def invalidate_model(self, task: str, model_version: str | None = None) -> int:
        ...
        ver = getattr(settings, "CACHE_SCHEMA_VERSION", 1)
        if model_version:
            pattern = f"crucible:cache:v{ver}:{domain}:*:m:{model_version}*"
        else:
            pattern = f"crucible:cache:v{ver}:{domain}:*"
        count = self.delete_pattern(pattern)
    ...
    def invalidate_entity(self, domain: str, entity_id: str) -> int:
        ...
        ver = getattr(settings, "CACHE_SCHEMA_VERSION", 1)
        pattern = f"crucible:cache:v{ver}:{domain}:*:{entity_id}:*"
        count = self.delete_pattern(pattern)
        # Strictly avoid broad cross-domain patterns like *:{entity_id}*
        return count

    def clear(self) -> None:
        """Clear all L1 entries and scan/delete all Crucible AI cache keys in L2."""
        self.l1.clear()
        ver = getattr(settings, "CACHE_SCHEMA_VERSION", 1)
        for k in self.l2.scan_iter(match=f"crucible:cache:v{ver}:*"):
            self.l2.delete(k)
        self._inc_metric("cache_invalidation")
