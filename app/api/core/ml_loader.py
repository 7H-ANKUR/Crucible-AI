"""app/api/core/ml_loader.py — Load all .joblib model artifacts at startup.

Load order for each artifact:
  1. Check local disk  (app/ml/artifacts/)
...
logger = logging.getLogger("crucible.ml")
MODELS: dict = {}
...
# Promotion and rollback call invalidate_model_metadata() explicitly, so this TTL
# is a backstop against a registry write that bypassed the application, not the
# mechanism by which promotions take effect. Ten seconds was shorter than a
# single cold page load, so the registry was re-read several times per request
# against a database ~1.2 s away.
_METADATA_TTL = 120.0  # seconds
