"""app/api/core/contracts.py — Canonical Pydantic schemas and contracts for Crucible AI API.
...
class ScenarioInterventionModel(BaseModel):
    # `model_backed` collides with pydantic's protected `model_` namespace.
    model_config = {"protected_namespaces": ()}
...
class ScenarioResponse(BaseModel):
    ...
    # Evidence quality replaces the former `confidence_tier` / `model_confidence`
    # pair. Those were two-branch constants presented as a calibrated percentage;
    # core/evidence.py measures support from freshness, feature completeness,
    # model availability, historical depth and constraint coverage instead.
    evidence_quality: str = "UNAVAILABLE"
    evidence_factors: list[dict] = Field(default_factory=list)
    evidence_limiters: list[str] = Field(default_factory=list)
    data_origin: str
    disclaimer: str
