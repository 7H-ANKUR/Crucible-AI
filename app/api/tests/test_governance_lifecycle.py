"""app/api/tests/test_governance_lifecycle.py — Strict governance approval gate & safe promotion tests."""
from fastapi.testclient import TestClient

from app.api.core.db import execute, query
...
def test_cannot_promote_without_prior_approval():
    ...
    rows = query(
        """INSERT INTO gov.model_registry
           ...
           VALUES ('production_forecast', 'lightgbm', 'test-unapproved-v1', 'challenger', 'PASS', 'app/ml/artifacts/production_forecast_champion.joblib')
           RETURNING id"""
    )
    model_id = rows[0]["id"]
