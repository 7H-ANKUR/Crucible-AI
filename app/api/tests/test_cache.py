"""app/api/tests/test_cache.py — Production-Grade Test Suite for Crucible AI Hybrid Tiered Cache.
...
def test_tiered_cache_l2_hit_and_miss_with_mock():
    ...
    with patch.object(mgr.l2, "_get_client", return_value=fake_redis):
        ...
        res1 = mgr.get_or_set("crucible:cache:v1:test:res", ttl=60.0, producer=produce)
        ...
        res2 = mgr.get_or_set("crucible:cache:v1:test:res", ttl=60.0, producer=produce)
...
def test_graceful_fallback_when_redis_fails():
    ...
    with patch.object(mgr.l2, "_get_client", return_value=failing_redis):
        ...
        val = mgr.get_or_set("crucible:cache:v1:resilience:test", ttl=30, producer=lambda: "authoritative_data")
        assert val == "authoritative_data"

        # L1 should still have received the entry
        hit, l1_val = mgr.l1.get("crucible:cache:v1:resilience:test")
        assert hit is True
