"""app/api/routers/training.py — Training run lifecycle management.
...
logger = logging.getLogger("crucible.training")
...
def _run_modal_training(
    ...
    try:
        ...
        import modal
        fn: Any = modal.Function.from_name("crucible-training", "run_training_job")
        ...
        for m in result.get("metrics", []):
            ...
            rows = query(
                ...
                (
                    ...
                    data_origin,
                    f"app/ml/artifacts/{art_filename}",
                    real_drive_id,
                    art_meta.get("sha256"),
        ...
        for task in tasks:
            ...
            rows = query(
                ...
                (
                    ...
                    data_origin,
                    f"app/ml/artifacts/{task}_{metrics.get('model','unknown')}.joblib",
                    run_id,
                )
            )
            if rows:
