...
logger = logging.getLogger("crucible.health")
...
@router.get("/health/engine", summary="Crucible AI engine health and bridge metrics")
def engine_health():
    """Health of the Crucible AI engine and of this gateway's bridge to it.

    Reported separately from /health so an engine outage never turns the
    platform's own readiness probe red — the engine powers the ML Lab, and the
    rest of Crucible AI serves normally without it.
    """
    ...
    return {
        ...
        "latency_ms": round(probe.elapsed_ms, 1) if probe.elapsed_ms else None,
