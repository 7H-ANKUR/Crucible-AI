"""app/api/routers/decisions.py — Decision Memory, State Machine & Task-Aware Outcome Tracking.

Endpoints:
  GET  /api/v1/decisions              — list decisions with recorded outcomes and lifecycle state
  POST /api/v1/decisions              — create a decision (starts in READY_FOR_REVIEW)
...
class CreateDecisionRequest(BaseModel):
    ...
    lifecycle_state: str = "READY_FOR_REVIEW"  # DRAFT | SIMULATED | READY_FOR_REVIEW
...
def record_decision(
    ...
    elif state not in ("DRAFT", "SIMULATED", "READY_FOR_REVIEW"):
        state = "READY_FOR_REVIEW"
...
def review_decision(
    ...
    with transaction() as tx:
        ...
        if current_state not in ("DRAFT", "SIMULATED", "READY_FOR_REVIEW"):
            raise HTTPException(
                422,
                f"Cannot review decision in state '{current_state}'. Only "
                f"'DRAFT', 'SIMULATED' or 'READY_FOR_REVIEW' decisions can be reviewed."
            )
...
def execute_decision(
    ...
    with transaction() as tx:
        ...
        tx.execute(
            """UPDATE gov.decisions
               SET lifecycle_state='COMPLETED', status='completed', executed_by=%s, executed_at=NOW()
               WHERE id=%s""",
...
def record_outcome(
    ...
    State requirement: Can only record outcomes for COMPLETED (or APPROVED) decisions.
    Evaluates:
    ...
    with transaction() as tx:
        ...
        current_state = dec[0]["lifecycle_state"]
        if current_state not in ("COMPLETED", "APPROVED"):
            raise HTTPException(
                422,
                f"Can only record outcomes for 'COMPLETED' (or 'APPROVED') decisions. Current state: '{current_state}'."
            )
        ...
        # Advance to MEASURED. `status` is constrained to lower(lifecycle_state)
        # by migration 002, so the two can no longer drift apart.
        tx.execute("UPDATE gov.decisions SET lifecycle_state='MEASURED', status='measured' WHERE id=%s", (decision_id,))
