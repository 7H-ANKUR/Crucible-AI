"""app/api/routers/intelligence.py — Cross-Domain Intelligence Operating Center.
...
@router.get("/pulse")
def get_mine_pulse(ctx: MineContext = Depends(mine_context)):
    """Computes dynamic operational health score across 5 Crucible AI pillars (memoised)."""
    mine_id = ctx.mine_id
...
def _compute_pulse(mine_id: str):
    """Computes dynamic operational health score across 5 Crucible AI pillars.
...
def get_top_issues(ctx: MineContext = Depends(mine_context)):
...
def nl_query(body: QueryRequest, user=Depends(get_current_user)):
    ...
    """
    q = body.query.lower()
    # Resolves, authorises, or refuses. Never substitutes an arbitrary mine.
    target_mine = resolve_mine_context(body.mine_id, user).mine_id
    # Phrase-first scoring, in core/intent.py. The chain this replaces listed
    # "target" under both production and exploration and tested production
    # first, so "which exploration target should we drill?" was answered with
    # tonnage figures.
    ...
    if intent == "production_risk":
        ...
        return {
            "query": body.query,
            "intent": intent,
            "headline": "Crucible AI Cross-Domain Operational Briefing",
            "evidence": [
