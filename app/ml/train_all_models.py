...
MODELS = [
    ("Model 1 - Prospectivity",      "app/ml/models/train_prospectivity.py"),
...
val_rows = []
all_fields = []
for p in sorted(pathlib.Path("app/ml/artifacts").glob("*_validation.csv")):
    with open(p) as f:
...
if val_rows:
    out = pathlib.Path("app/ml/FINAL_MODEL_VALIDATION.csv")
