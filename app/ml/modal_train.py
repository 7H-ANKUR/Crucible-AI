"""app/ml/modal_train.py — Serverless Cloud Model Retraining on Modal.com.
...
Usage:
  ...
  3. Deploy as a persistent serverless app (for API triggers):
     modal deploy app/ml/modal_train.py
"""
import hashlib
import json
import os
...
# Define Modal App
app = modal.App("crucible-training")

# Persistent storage volume for trained model artifacts
volume = modal.Volume.from_name("crucible-artifacts", create_if_missing=True)
...
training_image = (
    ...
    .add_local_dir("app/ml/models", remote_path="/root/app/ml/models")
)

DOMAIN_SCRIPTS = {
    "exploration": [
        ("Model 1 - Prospectivity Scorer", "app/ml/models/train_prospectivity.py")
    ],
    "production": [
        ("Model 2 - Production Forecast", "app/ml/models/train_production_forecast.py"),
        ("Model 3 - Shortfall Classifier", "app/ml/models/train_shortfall.py"),
    ],
    "equipment": [
        ("Model 4 - Equipment Failure", "app/ml/models/train_equipment_failure.py")
    ],
}
...
def run_training_job(
    ...
    art_dir = work_dir / "app/ml/artifacts"
    ...
    run_env = {**os.environ, "CRUCIBLE_DATA_ORIGIN": data_origin}
...
def main(domain: str = "production", all: bool = False):
    """Local CLI entrypoint for running via `modal run app/ml/modal_train.py`."""
