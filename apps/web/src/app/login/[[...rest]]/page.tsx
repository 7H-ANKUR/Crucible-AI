...
/**
 * Login — Clerk-powered sign-in/up styled to the MINEx theme.
...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (isSignedIn) router.replace('/production');
  }, [isSignedIn, router]);
    ...
    <main className="min-h-screen bg-[#08151c] flex flex-col items-center justify-center px-4 py-12 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md flex flex-col items-center gap-7">
        <Link href="/" className="flex items-center gap-2 group">
          ...
          <span className="font-['Space_Grotesk'] text-4xl font-bold text-accentt tracking-tight">MINEx</span>
        </Link>
        ...
        <div className="w-full flex justify-center">
          <SignIn
            fallbackRedirectUrl="/production"
            signUpForceRedirectUrl="/production"
            appearance={{
              variables: {
                colorPrimary: '#f59a23',
        ...
        <Link href="/production" className="text-xs font-bold text-ink3 hover:text-accentt transition-colors uppercase tracking-wider">
          Continue as guest →
        </Link>
      </div>
    </main>
