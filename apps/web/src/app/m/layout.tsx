...
import { canAccessPage, pagesForRole, ROLE_HOME, ROLE_LABEL, useMinexAuth, type Role } from '@/lib/roles';
...
export default function MobileLayout({ children }: { children: React.ReactNode }) {
  ...
  const { isSignedIn, role, isSuperAdmin } = useMinexAuth();
  const { user } = useUser();
  ...
  const setDesktopCookie = () => {
    document.cookie = 'minex_desktop=1; path=/; max-age=604800; samesite=lax';
    window.location.href = '/';
  };

  const goMobile = () => {
    document.cookie = 'minex_desktop=; path=/; max-age=0';
