...
import { useMinexAuth, pagesForRole, groupsForRole } from '@/lib/roles';
import { ThemeToggle } from '@/components/minex/ThemeToggle';
import { Avatar } from '@/components/minex/Avatar';

export function TopNavBar({ unreadAlertCount = 0 }: { unreadAlertCount?: number }) {
  const pathname = usePathname();
  const { role, isSuperAdmin, isSignedIn } = useMinexAuth();
    ...
    return (
      ...
      >
        <Link href="/" className="flex items-center gap-2 group">
          ...
          <span className="font-['Space_Grotesk'] text-2xl sm:text-[28px] font-bold text-accentt tracking-tight">MINEx</span>
        </Link>
    ...
    <>
      <nav id="app-topbar" className="sticky top-0 z-40 flex justify-between items-center w-full px-5 py-2 bg-panel/90 backdrop-blur-xl border-b border-line2/50 shadow-sm">
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <Link href="/" className="font-['Manrope'] text-xl font-black text-accentt hover:opacity-90 transition-opacity tracking-tight">MINEx</Link>
          <div className="hidden md:flex items-center gap-1 text-xs font-semibold">
            {pages.map((tab) => (
              <Link key={tab.href + tab.label} href={tab.href}
                className={`px-3 py-1.5 rounded-full transition-all ${pathname.startsWith(tab.href) ? 'bg-chipon text-inkb font-bold shadow-sm' : 'text-ink2 hover:text-ink hover:bg-frost/5'}`}>
                {tab.label}
              </Link>
            ))}
          </div>
        ...
        <div className="flex items-center gap-2">
          <ThemeToggle />
          <Link href="/alerts" className="p-2 text-ink2 hover:text-accentt hover:bg-panel4/50 rounded-full transition-colors relative" title="Alerts">
            <span className="material-symbols-outlined !text-[20px]">notifications</span>
            {unreadAlertCount > 0 && <span className="absolute top-1 right-1 w-2 h-2 bg-warn rounded-full border-page"></span>}
          </Link>
          {isSuperAdmin && (
        ...
        <div className="fixed inset-0 z-50 md:hidden" onClick={() => setDrawerOpen(false)}>
          ...
          <aside onClick={(e) => e.stopPropagation()} className="absolute top-0 left-0 bottom-0 w-[260px] bg-panel border-r border-line shadow-2xl flex flex-col animate-fadeIn">
            <div className="px-4 pt-4 pb-3 border-b border-line flex items-center justify-between">
              <span className="font-['Space_Grotesk'] text-lg font-bold text-accentt tracking-tight">MINEx</span>
...
export function SideNavBar(_props: { alertCount?: number } = {}) {
  const pathname = usePathname();
            ...
            {group.pages.map((item) => {
              ...
              return (
                <Link key={item.href + item.label} href={item.href} title={item.label}
                  className={`flex items-center gap-3.5 py-2 px-3 rounded-xl mx-0.5 transition-all whitespace-nowrap overflow-hidden ${
