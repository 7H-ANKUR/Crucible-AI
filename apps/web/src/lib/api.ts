/**
 * api.ts — Central API client for MINEx frontend.
 *
 * Token strategy:
 ...
 * build time:
 *   - production  → apps/web/.env.production (committed, holds the Render URL),
 *     or a Vercel project environment variable, which takes precedence.
 *   - development → apps/web/.env.local (gitignored, usually localhost:8000).
 *
  ...
  if (process.env.NODE_ENV === 'production') {
    throw new Error(
      'NEXT_PUBLIC_API_URL is not set. A production build must point at the deployed ' +
        'API (e.g. https://minex-backend-56rm.onrender.com). Set it in the Vercel ' +
        'project environment variables, or in apps/web/.env.production.'
    );
  }
...
export function setToken(token: string) {
  ...
  if (typeof window !== 'undefined') {
    try {
      sessionStorage.setItem('minex_active_token', token);
    } catch {}
  }
}
...
export function getToken(): string | null {
  ...
  if (typeof window !== 'undefined') {
    try {
      const stored = sessionStorage.getItem('minex_active_token');
      if (stored) {
        _activeToken = stored;
        return stored;
      }
...
export function clearToken() {
  ...
  if (typeof window !== 'undefined') {
    try {
      sessionStorage.removeItem('minex_active_token');
    } catch {}
  }
}
