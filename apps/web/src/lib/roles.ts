...
/**
 * roles.ts — MINEx role model + navigation IA.
 *
 * Roles:
...
export interface MinexAuthState {
  isSignedIn: boolean;
  role: Role | null;
  userId: string | null;
  name: string | null;
...
export const NAV_GROUPS: readonly NavGroup[] = [
  {
    label: 'OVERVIEW',
    pages: [
      { href: '/production',   label: 'Overview',     icon: 'dashboard',               dept: 'production' },
    ],
  },
  {
    label: 'EXPLORE',
  ...
  },
  {
    label: 'PLAN',
    pages: [
      { href: '/scenario',     label: 'Planning',     icon: 'compare_arrows',          dept: 'planning' },
    ],
  },
  {
    label: 'DECIDE',
    pages: [
      { href: '/intelligence', label: 'Intelligence', icon: 'psychology',              dept: 'all', isNew: true },
      { href: '/scenario',     label: 'Simulate',     icon: 'science',                 dept: 'planning' },
  ...
  },
  {
    label: 'DATA',
    pages: [
      { href: '/data-hub',     label: 'Data Hub',     icon: 'cloud_upload',            dept: 'all', isNew: true },
    ],
  },
  {
    label: 'TRUST',
...
const DEMO_AUTH: MinexAuthState = { isSignedIn: false, role: null, userId: null, name: null };
const AuthCtx = createContext<MinexAuthState>(DEMO_AUTH);

export function MinexAuthProvider({ value, children }: { value: MinexAuthState; children: ReactNode }) {
  return createElement(AuthCtx.Provider, { value }, children);
}

export function useMinexAuth(): MinexAuthState & { isSuperAdmin: boolean } {
...
export function canAccessPage(role: Role | null, pathname: string): boolean {
  ...
  if (path.startsWith('/alerts') || path.startsWith('/intelligence') || path.startsWith('/data-hub')) return true;
