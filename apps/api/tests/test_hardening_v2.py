"""apps/api/tests/test_hardening_v2.py — Test suite for v2 deep hardening invariants.
...
from apps.api.core.contracts import ServingStatus
from apps.api.core.ml_loader import ensure_active_model
...
def test_mapping_resets_validation_status():
    ...
    with patch("apps.api.routers.data_hub.query", return_value=fake_ver), \
         patch("apps.api.routers.data_hub.execute") as mock_exec:
        res = client.post(
...
def test_health_ready_sanitizes_errors():
    ...
    with patch("apps.api.routers.health.query", side_effect=Exception("FATAL: password authentication failed for user secret_user")):
...
def test_intelligence_pulse_null_fallback():
    ...
    with patch("apps.api.routers.intelligence.query", return_value=[]):
...
def test_alerts_mine_access_forbidden():
    ...
    with patch("apps.api.routers.alerts.check_mine_access", return_value=False):
