"""apps/api/tests/test_hardening_guarantees.py — Rigorous tests for Final Architectural Correctness Hardening.
...
from apps.api.core.ml_loader import (
    ensure_active_model,
...
from apps.ml import modal_train
...
def test_fail_safe_model_replacement():
    ...
    from apps.api.core.ml_loader import ACTIVE_SERVING_VERSIONS, MODELS
...
def test_scenarios_cross_mine_isolation():
    """Scenarios for non-existent mine return structured INSUFFICIENT_DATA, never fallback mine data."""
    headers = _get_auth_headers()
    ...
    assert data["data_quality"]["status"] == "INSUFFICIENT_DATA"
    assert data["baseline"]["production_t"] == 0.0
...
def test_transaction_context_manager_is_importable():
    ...
    from apps.api.core.db import transaction
...
def test_transaction_rollback_on_http_exception():
    ...
    create_res = client.post(
        "/api/v1/decisions",
        json={"problem": "TX rollback test", "recommendation": "Do nothing", "lifecycle_state": "RECOMMENDED"},
        headers=headers,
    )
...
def test_decision_state_machine_full_path():
    """Walk the full RECOMMENDED → APPROVED → EXECUTED → OUTCOME_RECORDED path."""
    headers = _get_auth_headers("super_admin")

    # Create
    res = client.post(
        "/api/v1/decisions",
        json={"problem": "SM-test", "recommendation": "SM-rec", "lifecycle_state": "RECOMMENDED"},
        headers=headers,
    )
    assert res.status_code == 200
    did = res.json()["decision_id"]
    assert res.json()["lifecycle_state"] == "RECOMMENDED"
    ...
    assert res.status_code == 200
    assert res.json()["lifecycle_state"] == "EXECUTED"
    ...
    assert res.status_code == 200
    data = res.json()
    assert data["lifecycle_state"] == "OUTCOME_RECORDED"
...
def test_execute_requires_approved_state():
    ...
    res = client.post(
        "/api/v1/decisions",
        json={"problem": "Execute-guard test", "recommendation": "rec", "lifecycle_state": "RECOMMENDED"},
        headers=headers,
    )
    did = res.json()["decision_id"]
...
def test_outcome_requires_executed_state():
    ...
    res = client.post(
        "/api/v1/decisions",
        json={"problem": "Outcome-guard test", "recommendation": "rec", "lifecycle_state": "RECOMMENDED"},
        headers=headers,
    )
    did = res.json()["decision_id"]
...
def test_decision_effectiveness_production_forecast_metric():
    ...
    res = client.post(
        "/api/v1/decisions",
        json={"problem": "Effectiveness test", "recommendation": "rec", "lifecycle_state": "RECOMMENDED"},
        headers=headers,
    )
    did = res.json()["decision_id"]
...
def test_data_hub_approve_requires_validated_status():
    ...
    with patch("apps.api.routers.data_hub.query", return_value=fake_version), \
         patch("apps.api.routers.data_hub.check_domain_access", return_value=True):
        res = client.post(
