"""apps/api/core/config.py — Application settings with production safety guards."""
import logging
import pathlib

from pydantic_settings import BaseSettings, SettingsConfigDict

logger = logging.getLogger("minex.config")
...
class Settings(BaseSettings):
    ...
    ML_ARTIFACTS_DIR: str  = "apps/ml/artifacts"
...
settings = Settings()
ARTIFACTS = pathlib.Path(settings.ML_ARTIFACTS_DIR)
...
def validate_production_settings(cfg: Settings | None = None) -> None:
    ...
    weak_keys = {
        "minex-sih26009-secret-change-in-prod",
