---
title: MINEx Mining Intelligence Platform
emoji: ⛏️
colorFrom: blue
colorTo: indigo
sdk: gradio
...
MINEx is a governed, AI-driven mine operating system that fuses four real ML models, geospatial intelligence, a constraint-aware scenario engine, and a full data governance lifecycle into a single Next.js glassmorphic interface.
...
- An **Intelligence Center** for cross-domain NL reasoning, mine pulse, and root cause analysis
- A **Decision Memory** system that tracks decisions and measures their real outcomes
- Full **RBAC** with role-gated write endpoints across all 10 API routers
...
All values sourced from `apps/ml/FINAL_MODEL_VALIDATION.csv`. Not overridden by synthetic defaults.

---

## 🚀 Platform Capabilities

### Production Command Center (`/production`)
- P10/P50/P90 forecast with **separate** Forecast Gap and Shortfall Risk signals
...
| **Cloud Training** | Modal.com serverless GPU/CPU containers (`apps/ml/modal_train.py`) |
...
├── seed_db.py
├── MINEX_CREDENTIALS.md        # Local dev credentials (gitignored)
└── README.md
```
...
DEMO_ADMIN_PASSWORD=your-password    # see MINEX_CREDENTIALS.md
...
MINEx implements a production-hardened hybrid caching layer designed for multi-instance Render deployments:
...
MINEx supports remote model training using [Modal.com](https://modal.com) serverless GPU/CPU containers (`apps/ml/modal_train.py`).
