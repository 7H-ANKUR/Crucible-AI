/**
 * lib/crucible.ts — Shared types, India-corrected fallback data, and API mappers
 * for the Crucible AI Stitch-themed frontend. Every screen renders from live API data
...
export function mapForecastToBars(
  ...
  // Empty, not a fabricated curve. FALLBACK_FORECAST rendered invented
  // tonnages (3.2 t, 3.6 t, ...) complete with a peak marker, and nothing
  // in the UI distinguished them from measured output.
  return grid;
}
...
export function mapTargets(api: any): ExplorationTarget[] {
  ...
  // No targets is a finding, not a reason to show example targets.
...
export function mapLedger(api: any): PredictionLedgerEntry[] {
  ...
  // An empty ledger means nothing has been predicted yet. Saying so is
  // more useful than showing entries that were never recorded.
  if (!rows.length) return [];
